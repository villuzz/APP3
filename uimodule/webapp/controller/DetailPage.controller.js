sap.ui.define([
  ...
  return Controller.extend("PM030.APP3.controller.DetailPage", {
    ...
    onCancelAEConfirm: async function () {
      var sel = this.getView().byId("tAzioni").getSelectedItems();
      var AZIONI = this.getView().getModel("AzioniDetail").getData();

      // Logiche per cancellazione AE se non presenti nella tabella T_CONS_AE
      var aFiltersConsAe = [];
      var aCont = sel.map(row => row.getBindingContext("AzioniDetail").getObject().Cont);

      aCont.forEach(row => {
        aFiltersConsAe.push(new Filter("Cont", FilterOperator.EQ, row));
      });
      var aFilterPartial = new Filter({
        filters: aFiltersConsAe,
        and: false
      });
      var aConsAe = await this._getTable("/T_CONS_AE", [aFilterPartial]);

      if (aConsAe.length > 0) {
        MessageBox.warning("Non è possibile eliminare. Ci sono record nella tabella ZPM4R_T_CONS_AE");
        return;
      }

      //this.onCancelAEConfirm(); //"1094".padStart(10, "0");
    ...
    onSpostaAE: async function (sText) {
      var sel = this.getView().byId("tAzioni").getSelectedItems();
      var AZIONI = this.getView().getModel("AzioniDetail").getData();
      // var deleteRecord = oEvent.getSource().getBindingContext("sSelect").getObject();
      for (var i = (sel.length - 1); i >= 0; i--) {
        ...
        // Ottengo il testo esteso da salvare nel nuovo indice
        var sTesto = await this.onTestoEstesoAE(line);
        ...
        await this.onTestoEstesoAESave(line, sTesto);
