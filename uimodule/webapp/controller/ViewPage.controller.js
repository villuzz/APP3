sap.ui.define([
  ...
  '../model/formatter',
], function (Controller, JSONModel, MessageBox, viewPiani, TablePersoController, Filter, FilterOperator, MessageToast, Token, exportLibrary, coreLibrary, xlsx, formatter) {
  "use strict";
  var oResource;
  ...
  return Controller.extend("PM030.APP3.controller.ViewPage", {
    ...
    onInit: function () {
      ...
      var oinEquipment = this.getView().byId("inEquipment");  // Equipment
      var oinEquipmentActEl = this.getView().byId("inEquipmentActEl"); // Equipment AE
      var oinITplnrActEl = this.getView().byId("inITplnrActEl"); // Sede Tecnica AE
      ...
      oinEquipment.addValidator(fnValidator);
      oinEquipmentActEl.addValidator(fnValidator);
      oinITplnrActEl.addValidator(fnValidator);
    ...
    onDataExport: async function () {
      ...
      if (items.length !== 0) {
        for (var i = 0; i < items.length; i++) {
          ...
          // Skip ripetuti nel caso di visualizzazione con azione elementare
          if (this.byId("switchFilterVisualId").getState()) {
            if (aPresente.includes(line.IndexPmo)) {
              continue;
            } else {
              aPresente.push(line.IndexPmo);
            }
          }
          ...
          for (var i = 0; i < aTable.length; i++) {
            if (aTable[i].Cont === sLine.Cont && aTable[i].GestioneLoad === "X") {
              ...
              if (aTable[i].IntegTxtEsteso !== "" && aTable[i].IntegTxtEsteso !== undefined) {
                // 2DO da testare, dentro la linea abbiamo il vero testo esteso non bisogna prenderlo dalla get
                await this.onTestoEstesoAESave(aTable[i], aTable[i].IntegTxtEsteso);
                aTable[i].IntegTxtEsteso = "X";
              } else {
                var sTestoEstesoToSave = await this.onTestoEstesoAE(aTable[i]); //aTable[i].IntegTxtEsteso
                await this.onTestoEstesoAESave(aTable[i], sTestoEstesoToSave);
    ...
    onSearchFilters: async function () {
      ...
      // Gestione filtro ist, nel caso vengano rimossi a mano si buggano da gestire
      var aIstComponente = this.getView().getModel("SelSedeReale") ? this.getView().getModel("SelSedeReale").getData().map(row => row.Funcloc) : [];
      aIstComponente.forEach(row => {
        aFilters.push(new Filter("IStComponente", FilterOperator.EQ, row));
      });
      if (sFilter.IStComponente !== undefined && sFilter.IStComponente !== "") {
        // aFilters.push(new Filter("IStComponente", FilterOperator.Contains, sFilter.IStComponente));
      }
      ...
      if (this.getView().byId("inEquipment").getTokens().length > 0) {
        aSelIndici = this.getView().byId("inEquipment").getTokens();
        aSelInd = [];
        for (i = 0; i < aSelIndici.length; i++) {
          aSelInd.push(aSelIndici[i].getProperty("key"));
        }
        tempFilter = this.multiFilterText(aSelInd, "IEquipmentCompo");
        aFilters = aFilters.concat(tempFilter);
      }
      if (this.getView().byId("inEquipmentActEl").getTokens().length > 0) {
        aSelIndici = this.getView().byId("inEquipmentActEl").getTokens();
        aSelInd = [];
        for (i = 0; i < aSelIndici.length; i++) {
          aSelInd.push(aSelIndici[i].getProperty("key"));
        }
        tempFilter = this.multiFilterText(aSelInd, "IEquipmentActEl");
        aFilters = aFilters.concat(tempFilter);
      }
      if (this.getView().byId("inITplnrActEl").getTokens().length > 0) {
        aSelIndici = this.getView().byId("inITplnrActEl").getTokens();
        aSelInd = [];
        for (i = 0; i < aSelIndici.length; i++) {
          aSelInd.push(aSelIndici[i].getProperty("key"));
        }
        tempFilter = this.multiFilterText(aSelInd, "ITplnrActEl");
        aFilters = aFilters.concat(tempFilter);
      }
      ...
      if (sFilter.descrComponente !== undefined && sFilter.descrComponente !== "") {
        aFilters.push(new Filter("IDesComponente", FilterOperator.Contains, sFilter.descrComponente));
      }

      var aFilterFE = [];
      if (sFilter.descrComponente !== undefined && sFilter.descrComponente !== "") {
        // aFilterFE.push(new Filter("IDesComponente", FilterOperator.Contains, sFilter.descrComponente));
      }
      ...
      var sIndex = sPiani[0].T_PMOSet.results;  // Tabella principale
      var sAzioni = sPiani[0].T_ACT_ELSet.results; // Dati da replicare

      var bState = this.byId("switchFilterVisualId").getState();

      if (bState) {
        var aIndex2 = [];
        // Controllo per duplica per contatore
        sIndex.forEach(singleIndex => {
          sAzioni.forEach(singleAzione => {
            if (singleIndex.IndexPmo === singleAzione.IndexPmo) {
              var oToPush = JSON.parse(JSON.stringify(singleIndex));  // Risolve il problema del puntamento
              oToPush.Cont = singleAzione.Cont;
              aIndex2.push(oToPush);
            }
          });
        });
        sIndex = aIndex2;
      ...
      this.byId("col100").setVisible(bState);
    ...
    /* SEZIONE GESTIONE SCHEDA TECNICA REALE*/
    ...
    onConfirmSedeTecnicaReale: function () { // this._sInputId.setValue(null);
      ...
      var aToken = [];
      aSel.forEach(oSel => {
        aToken.push(new Token({ text: oSel.Funcloc, key: oSel.Funcloc }))
      });

      // Pppolo la lista 
      var oSchedaTecnicaMultiInputId = this.byId("schedaTecnicaMultiInputId");
      oSchedaTecnicaMultiInputId.setTokens(aToken);
    ...
    onFilterSedeTecnicaReale: async function () {
      ...
      // var sSelect = this.getView().getModel("sSelect").getData();
      // var sSedeTecnica = await this._getTableDistinct("/Index_Azioni", [], "SEDE_TECNICA");
      // Timeout exception nel caso di *
      /*ListFl.N_CategoryRa = [{
        Sign: "I",
        Option: "EQ",
        Low: "H" // sSedeTecnica.SEDE_TECNICA //sSelect.SEDE_TECNICA
      }];
      */
      ...
      try {
        ...
        MessageBox.warning("Errore nella chiamata"); // + e
        sap.ui.core.BusyIndicator.hide();
        return;
      }
